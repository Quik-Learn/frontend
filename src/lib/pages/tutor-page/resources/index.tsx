...
import { dashboardItems } from '~/lib/utils/data';
...
const Courses = () => {
  ...
  const CourseCard = ({
    ...
    <Box
      ...
      onClick={() => router.push(`/tutor/resources/${id}`)}
    >
      <Image
        src={imageSrc}
        alt={title}
      ...
      <VStack p={4} align="center" justify={'center'} gap={4} h={'100%'}>
        ...
        <HStack spacing={1} justifyContent={'flex-end'} alignItems={'flex-end'}>
  ...
  return (
    <TutorContainer>
      ...
      <Pagination
        ...
        next={next}
        previous={previous}
        onPageChange={(page: number) => getCourses({ page_size: page })} // Optional: Callback if you need to handle page change externally
      />
    </TutorContainer>
  );
};
